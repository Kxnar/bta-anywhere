...
Status: **experimental; not merge eligible**. This record covers the isolated
`codex/crash-fault-injection` branch. It distinguishes synthetic checks from
checks that still need a disposable BTA client world. The root `HANDOFF.md` is
historical and is not the source of current Git or test status.
...
| Benchmarks | FAIL | Same-machine pre-launch baseline/candidate timing is absent. |
| Performance | FAIL | The 5% pre-launch threshold cannot yet be evaluated. |
